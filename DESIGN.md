...
**The Crisp Motion Rule.** All motion draws from four durations (`--motion-micro` 100ms, `--motion-state` 160ms, `--motion-nav` 220ms, `--motion-wipe` 420ms) and three curves (`--ease-travel` cubic-bezier(0.2,0,0,1) for mechanical travel, `--ease-arrive` cubic-bezier(0.16,1,0.3,1) for arrivals, `--ease-exit` for departures) — never springy, bouncy, or decoratively looping. Every authored move degrades under `prefers-reduced-motion` to opacity-only or nothing, without losing state feedback.

**The Bolted Chassis Rule (motion).** The chassis — top status bar, bottom control strip — never moves; the plate stack (`<main>`) travels. Each navigation names its spatial meaning on `html[data-motion]` (set by the direction module in `app.js`): **push-fwd/push-back** between sibling tabs by control-strip key order; **cover** (260ms rise from below) descending into a focused tool or detail plate; **uncover** (settle up from beneath at 98% scale) surfacing back to the desk; anything unrelated stays instant. Fade is reserved for relationships that aren't spatial — it is never the default transition.
