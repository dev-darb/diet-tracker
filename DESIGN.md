...
**The Crisp Motion Rule.** All motion draws from four durations (`--motion-micro` 100ms, `--motion-state` 160ms, `--motion-nav` 320ms, `--motion-wipe` 420ms) and three curves (`--ease-travel` cubic-bezier(0.2,0,0,1) for mechanical travel, `--ease-arrive` cubic-bezier(0.16,1,0.3,1) for arrivals, `--ease-exit` for departures) — never springy, bouncy, or decoratively looping. Every authored move degrades under `prefers-reduced-motion` to opacity-only or nothing, without losing state feedback.

**The Bolted Chassis Rule (motion).** The chassis — top status bar, bottom control strip — never moves; the plate stack (`<main>`) travels, and it travels as a **pair**: the outgoing plate is cloned across the body swap into a fixed clip layer (`.plate-ghost`, built by `app.js`) so old and new move together like a native OS stack. `html[data-motion]` names the meaning: **push-fwd/push-back** between sibling tabs by control-strip key order (incoming slides full-width, outgoing parallaxes away at a third speed and dims); **cover** — a focused tool rises over the desk, which visibly stays beneath, receding to 97%; **uncover** — the tool slides off downward, the desk brightening back to meet you. Anything unrelated stays instant. Fade is reserved for relationships that aren't spatial — it is never the default transition.
