...
- **Tranche 6 — the front door (Aug 2026): ✅ COMPLETE.** Welcome, all six auth screens, onboarding, profile and settings now render in the console world: chassis ground, one plate, the shared `<x-app.field>` anatomy (silkscreen label + seated well + `high`-signal error), console keys, and the `foody` nameplate as the one place the brand speaks. The starter kit's vocabulary is fully retired — Flux inputs/buttons/modals, the settings nav layout, the logo mark and the auth card/split layouts are deleted, and the account-delete modal became an in-world disclosure (the console has no modal grammar). Fixed in passing: `<x-app.console-key>` hard-coded `type="button"` before merging attributes, so a duplicate `type` resolved to the first and would have swallowed every auth form's submit.
- **Design programme:** all six tranches complete. Remaining design-adjacent backlog: an M8 backfill command for images/categories on products imported before Aug 2026.
