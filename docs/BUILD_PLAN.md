...
- **Next design tranches (founder-agreed, queued):** (4) the **resident chef** — chef stays IN the pantry as a compact component of the stock face: time-of-day-aware ("the likeliest meal you're having now"; later, user-set meal times), background-generated against a durable artifact, each suggestion carrying a one-tap "Cooked this" that prefills the home-cooked log; (5) outcome rewards (target-hit state on the Home scale, streak completion, day-close moment); (6) front-door console pass (welcome/auth/onboarding/profile/settings out of the emerald starter dialect).
