...
new #[Layout('components.layouts.app', ['title' => 'Home'])] class extends Component
{
    ...
    <div class="space-y-3">
        ...
        <section class="module px-5 pb-5 pt-4">
            ...
            <p class="mt-5 flex items-baseline justify-center gap-3">
                <span class="font-seg text-[3.4rem] leading-none {{ $today['has_data'] && $kcal !== null ? 'text-[#f2ede2]' : 'text-ink-faint' }}"
                ...
                <span class="data text-sm text-ink-dim">KCAL</span>
            </p>
            ...
            @if ($today['has_data'])
                <p class="data mt-4 border-t border-seam pt-3 text-[11px] tracking-[0.08em] text-ink-faint uppercase">
                    {{ $today['food_variety'] }} {{ $today['food_variety'] === 1 ? 'food' : 'foods' }} logged today
                </p>
            @else
        ...
        @if ($today['has_data'])
            @php($macros = collect(['protein' => 'Protein', 'carbs' => 'Carbs', 'fat' => 'Fat'])
                ->map(fn ($label, $key) => ['label' => $label, 'value' => $today['totals'][$key]]))
                ...
                @foreach ($macros as $m)
                    <div class="px-4 py-3.5">
                        ...
                        <div class="meter mt-2.5">
                            <span style="width: {{ $m['value'] !== null && $macroMax > 0 ? round($m['value'] / $macroMax * 100) : 0 }}%"></span>
                        </div>
                    </div>
                @endforeach
            </section>
